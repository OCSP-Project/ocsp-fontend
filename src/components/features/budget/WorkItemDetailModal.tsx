...
export function WorkItemDetailModal({ workItem, onClose, onUpdate }: WorkItemDetailModalProps) {
  ...
  const handleStatusChange = async (newStatus: WorkItemStatus) => {
    try {
      ...
      await workItemService.update(currentWorkItem.id, { status: statusMap[newStatus] });
      const updated = await workItemService.getById(currentWorkItem.id);
      setCurrentWorkItem({ ...updated, status: parseWorkItemStatus(updated.status) });
      onUpdate?.();
    } catch (error: any) {
  ...
  return (
    <div ref={modalRef} className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6">
      {/* Backdrop */}
      <div
        ref={overlayRef}
        ...
        <div className="flex-shrink-0 px-6 sm:px-8 py-5 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
          <div className="flex items-start sm:items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2 line-clamp-2">{currentWorkItem.name}</h2>
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                <span className={`px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${statusColors.bg} text-white shadow-lg`}>
                  {statusColors.label}
                </span>
...
function getCurrentUser(): { id: string; fullName: string; avatar?: string } | null {
...
function OverviewTab({ workItem, onStatusChange, onProgressChange, onAssignedUsersChange, isUpdating = false }: OverviewTabProps) {
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <InfoCard label="Giai đoạn" value={workItem.phase || workItem.type || 'San nền'} />
          <InfoCard label="Chế độ" value={workItem.mode || 'Nội bộ'} valueClass="text-green-600" />
          <InfoCard label="Thời gian" value={`${calculateDuration()} ngày`} badge />
          <InfoCard label="Ngày bắt đầu" value={formatDate(workItem.startDate)} badge badgeColor="blue" />
          <InfoCard label="Ngày kết thúc" value={formatDate(workItem.endDate)} badge badgeColor="pink" />

          <div>
            <label className="text-sm font-medium text-gray-700 block mb-2">Trạng thái</label>
            {onStatusChange ? (
              <StatusDropdown
                currentStatus={typeof workItem.status === 'string' ? parseWorkItemStatus(workItem.status) : workItem.status}
                onStatusChange={onStatusChange}
                disabled={isUpdating}
              />
            ) : (
              <span className={`inline-block px-3 py-1.5 rounded-lg text-sm font-medium text-white ${getStatusColor(typeof workItem.status === 'string' ? parseWorkItemStatus(workItem.status) : workItem.status, workItem.progress).bg}`}>
                {getStatusColor(typeof workItem.status === 'string' ? parseWorkItemStatus(workItem.status) : workItem.status, workItem.progress).label}
              </span>
            )}
          </div>

          <div>
            <label className="text-sm font-medium text-gray-700 block mb-2">Tiến độ</label>
            {onProgressChange ? (
              <div className="max-w-[200px]">
                <ProgressInput
                  currentProgress={workItem.progress || 0}
                  status={typeof workItem.status === 'string' ? parseWorkItemStatus(workItem.status) : workItem.status}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-gray-700">{workItem.progress ?? 0}%</span>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Overdue Warning */}
      {workItem.endDate && new Date(workItem.endDate) < new Date() && (workItem.progress ?? 0) < 100 && (
        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="text-red-600 font-semibold">
              Quá hạn {Math.ceil((new Date().getTime() - new Date(workItem.endDate).getTime()) / (1000 * 60 * 60 * 24))} ngày
            </span>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          ...
          <InfoCard
            ...
            value={`${workItem.plannedQuantity?.toLocaleString() || '0'}${workItem.unit ? ` ${workItem.unit}` : ''}`}
            valueClass="text-blue-600"
          />
          <InfoCard
            label="Khối lượng thực tế"
            value={`${workItem.actualQuantity?.toLocaleString() || '0'}${workItem.unit ? ` ${workItem.unit}` : ''}`}
        ...
        {workItem.description && (
          <div className="mt-4 pt-4 border-t border-gray-200">
            ...
            <p className="text-gray-700 leading-relaxed">{workItem.description}</p>
          </div>
        )}
      </div>
      ...
      {workItem.createdBy && (
        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <InfoCard label="Người giao" value={workItem.createdBy.fullName || workItem.createdBy.username || 'N/A'} />
            <InfoCard label="Ngày giao" value={formatDateTime(workItem.createdAt)} />
          </div>
        </div>
      )}
            ...
            {workItem.documents.map((doc) => (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <svg className="w-5 h-5 text-gray-600 group-hover:text-blue-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-800 truncate group-hover:text-gray-900">{doc.fileName}</p>
                    <p className="text-xs text-gray-500">{(doc.fileSize / 1024).toFixed(2)} KB</p>
                  </div>
                </div>
                <svg className="w-5 h-5 text-gray-500 group-hover:text-blue-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function InfoCard({ label, value, valueClass = 'text-gray-900', badge = false, badgeColor = 'gray' }: InfoCardProps) {
  ...
  return (
    <div>
      ...
      {badge ? (
        <span className={`inline-block px-3 py-1.5 rounded-lg text-sm font-medium ${badgeColors[badgeColor]}`}>
          {value}
        </span>
      ) : (
...
function UserCard({ user, color = 'blue' }: UserCardProps) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200 hover:border-gray-300 transition-all duration-200">
      <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${colors[color]} flex items-center justify-center text-white font-medium flex-shrink-0`}>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className={`rounded-xl p-6 border ${totalBudget - totalActual >= 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
          <h4 className={`text-sm mb-2 ${totalBudget - totalActual >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>Chênh lệch</h4>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Hạng mục</th>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">Đơn giá</th>
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">Thành tiền</th>
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">Thực tế</th>
              </tr>
            </thead>
              ...
              {workItem.budgets?.map((budget) => (
                <tr key={budget.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-800">{budget.category}</div>
                    {budget.description && <div className="text-sm text-gray-600">{budget.description}</div>}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-700">{budget.budgetType}</td>
                  <td className="px-6 py-4 text-sm text-gray-800 text-right whitespace-nowrap">
                    {budget.quantity} {budget.unit}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-800 text-right whitespace-nowrap">{formatCurrency(budget.unitPrice)}</td>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200 text-center">
        ...
        <p className="text-gray-600 text-lg">Chức năng thống kê đang được phát triển</p>
      </div>
    </div>
  );
}
...
function PaymentsTab({ workItem }: { workItem: WorkItemDetailDto }) {
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200 text-center">
        <div className="text-6xl mb-4">💳</div>
        <p className="text-gray-600 text-lg">Chức năng thu chi đang được phát triển</p>
      </div>
    </div>
  );
}
        ...
        {workItem.activities && workItem.activities.length > 0 ? (
          <div className="space-y-4">
            {workItem.activities.map((activity) => (
              <div key={activity.id} className="flex gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div className="flex-shrink-0 w-2 h-2 mt-2 bg-blue-500 rounded-full" />
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium text-gray-800">{activity.performedByName}</span>
                    <span className="text-xs text-gray-500">{formatDateTime(activity.performedAt)}</span>
                  </div>
                  <p className="text-sm text-gray-700">{activity.description}</p>
                  <span className="inline-block mt-2 px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded">{activity.activityType}</span>
                </div>
              </div>
            ))}
          </div>
